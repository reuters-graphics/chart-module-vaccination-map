...
function transition_attr(name, value) {
  var fullname = d3Selection.namespace(name), i = fullname === "transform" ? interpolateTransformSvg : interpolate;
...
var VaccineMap = /*#__PURE__*/function () {
  function VaccineMap() {
    _classCallCheck(this, VaccineMap);
  ...
  _createClass(VaccineMap, [{
    ...
    key: "_drawCountries",
    value: function _drawCountries(country, all, highlight) {
      var _all$props = all.props(),
          globe = _all$props.globe;
    ...
    value: function draw() {
      ...
      var filteredCountryKeys = useData.map(function (d) {
        return d.countryISO;
      });
      this._disputedBorders = topojson.mesh(topology, disputedBoundariesFeatures);
      this._land = topojson.feature(topology, landFeatures);
      var countries = topojson.feature(topology, countriesFeatures);
      var filteredCountries = countries.features.filter(function (d) {
        return filteredCountryKeys.includes(d.properties.isoAlpha2);
      });
      var numberScale = d3.scaleLinear().domain(d3.extent(useData, function (d) {
      ...
      var drawMap = function drawMap(projectedCentroid, highlighted) {
        _this._context.clearRect(0, 0, width, width);
      ...
      var chosenCountry = function chosenCountry() {
        destination = selectedCountry.properties.centroid;
        var projectedCentroid = projection(destination);
        drawMap(projectedCentroid, selectedCountry);
        _this._rotation = projection.rotate();
